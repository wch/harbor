...
docker_cmd.gce_instance <- function(host, cmd = NULL, args = NULL,
  ...
  if (capture_text) {
    ...
    googleComputeEngineR::gce_ssh(host$name, paste(cmd_string, ">", temp_remote), ...)
    googleComputeEngineR::gce_ssh_download(host$name, temp_remote, temp_local, ...)

    text <- readLines(temp_local, warn = FALSE)
    return(text)

  } else {
    return(googleComputeEngineR::gce_ssh(host$name, ..., cmd_string))
  }
